...
export class SeparacionProductosComponent {
 ...
 //END INPUT
  ...
  generarPDF() {
    ...
    // Encabezado
    doc.setFontSize(12);
    doc.text(`Pedido Produccion N°: ${this.presupuestoAAcceder?.id}`, 10, 20)
    doc.setFontSize(9);
    ...
    if (this.currentCliente) {
      doc.text(`Taller: ${this.currentCliente.razonSocial}`, 10, 30);
      doc.text(`Fecha: ${this.formatearFecha(this.fechaPresupuesto)}`, 10, 40);
  ...
  // Verifica si no es presupuestoCliente, si es así, usa negrita
    ...
    // Guardar el archivo
    doc.save(`Pedido-Produccion-Original-${this.currentCliente?.razonSocial}_${this.formatearFecha(this.currentPresupuesto?.fecha)}.pdf`);
  }
