...
generarPDFOriginal() {
  ...
  // Encabezado
  doc.setFontSize(12);
  doc.text(`Pedido Produccion N°: ${this.pedidoProduccionAAcceder?.id}`, 10, 20)
  doc.setFontSize(9);
  ...
  if (this.currentTaller) {
    ...
    doc.text(`Fecha: ${this.formatearFecha(this.fechaPedidoProduccion)}`, 10, 40);
...
// Verifica si no es presupuestoCliente, si es así, usa negrita
  ...
  // Guardar el archivo
  doc.save(`Pedido-Produccion-Original-${this.currentTaller?.razonSocial}_${this.formatearFecha(this.curentPedidoProduccion?.fecha)}.pdf`);
}
...
generarPDFPendientes() {
  ...
  const columnas = ['Código', 'Cantidad Pendiente', 'Descripción'];
  const columnWidths =[35, 33, 130];
...
// Verifica si no es presupuestoCliente, si es así, usa negrita
  ...
  this.mapaPresupuestoArticulos?.forEach((presupuestosArticulos, clave) => {
    ...
    const cantidadesPend = presupuestosArticulos.map(pa => pa.cantidadPendiente);
    const totalCantidadPend = cantidadesPend.reduce((acc, c) => (acc || 0) + (c || 0), 0);
    const descripcion = presupuestosArticulos[0].articulo?.descripcion || '';
    const descripcionCompleta = presupuestosArticulos.map(pa => `${pa.cantidadPendiente || 0}${pa.articulo?.color?.codigo + " " || ''}`).join('');
    const fila =[clave, totalCantidadPend, descripcion + ' ' + descripcionCompleta];
  ...
  // Guardar el archivo
  doc.save(`Pedido-Produccion-Pendientes-${this.currentTaller?.razonSocial}_${this.formatearFecha(this.curentPedidoProduccion?.fecha)}.pdf`);
}
