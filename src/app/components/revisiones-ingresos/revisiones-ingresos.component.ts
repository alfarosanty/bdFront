...
@Component({
  selector: 'app-revisiones-ingresos',
  templateUrl: './revisiones-ingresos.component.html',
  styleUrls: ['./revisiones-ingresos.component.css'],
  animations: [
...
export class RevisionesIngresosComponent {
...
// DATOS TABLA
columnsToDisplay = ['Ingreso', 'Imprimir'];
articuloColumnsToDisplay = ['Articulo', 'Descripcion' ,'Cantidad'];
...
seleccionarTaller(): void {
  if (this.currentTaller) {
    ...
    this.buscarIngresoXTaller();
  }
}



buscarIngresoXTaller(){
  if(this.currentTaller){
   this.ingresoService.getByTaller(this.currentTaller?.id).subscribe({
    next: (data) => {
      this.ingresosXTaller = data;
      this.ingresosXTallerFiltrados= data.sort((a, b) => new Date(a.fecha!).getTime() - new Date(b.fecha!).getTime());
...
aplicarFiltros(listaAFiltrar: Ingreso[]) {
...
filtrarPedidosProduccionXRangoFechas(listaIngresos: Ingreso[]): Ingreso[] {
  console.log("DEBERÍA ESTAR FILTRANDO POR FECHAS");

  if (this.fechaInicio && this.fechaFin) {
    const ingresosFiltradosEntreFechas = listaIngresos.filter(pedidoProduccion =>
      this.estaEntreFechas(this.fechaInicio, this.fechaFin, pedidoProduccion.fecha)
    );
    return ingresosFiltradosEntreFechas;
  }
...
drop(event: CdkDragDrop<Ingreso[]>) {
